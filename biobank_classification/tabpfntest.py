...
X, y = np.random.randn(215000, 20), np.random.randint(0, 2, 215000)
...
for leaf_index in unique_leaves:
    ...
    leaf_classifier = TabPFNClassifier(
        device="cuda", n_estimators=32, ignore_pretraining_limits=True
    )
...
for i, leaf_index in enumerate(test_leaf_indices):
