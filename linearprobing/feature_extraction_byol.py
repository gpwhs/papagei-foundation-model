...
from utils import (
    load_model_without_module_prefix,
    batch_load_signals,
...
def compute_signal_embeddings(
    ...
    with torch.inference_mode():
        for i in range(0, len(segments), batch_size):
            ...
            features = (
                extract_byol_features(model, batch_signal, device)
                .cpu()
                .detach()
                .numpy()
            )
...
def save_embeddings(
    ...
    for i in tqdm(range(len(child_dirs))):
        ...
        embeddings = compute_signal_embeddings(
            model=model,
            path=path,
            case=case,
            segments=segments,
...
if __name__ == "__main__":
    ...
    model = BYOL(image_size=1250, net=net)
    model = load_model_without_module_prefix(
        model,
        "../../models/2024_09_16_14_19_52/resnet_byol_mkhl43_2024_09_16_14_19_52_step10000_loss1.2126.pt",
    )
    device = f"cuda:{args.device}"
    model.to(device)

    if args.dataset in ["vital", "mimic", "mesa", "wesad", "dalia"]:
        df_train, df_val, df_test, case_name, ppg_dir = get_data_info(
            args.dataset, prefix="../", usecolumns=["segments"]
        )
    else:
        df_train, df_val, df_test, case_name, ppg_dir = get_data_info(
            args.dataset, prefix="../"
