...
def get_data_for_ml(df, dict_embeddings, case_name, label, level="patient"):
    ...
    X = np.vstack(
        [
            k.cpu().detach().numpy() if type(k) is torch.Tensor else k
            for k in dict_embeddings.values()
        ]
    )
    y = np.hstack(y)
...
def extract_labels(y, label, binarize_val=None):
    ...
    if (
        label == "Diabetes"
        or label == "cerebrovascular disease"
        or label == "cerebral infarction"
    ):
        y = np.where(y == "0", 0, 1)
...
def bootstrap_metric_confidence_interval(
    y_test, y_pred, metric_func, num_bootstrap_samples=500, confidence_level=0.95
):
...
def load_linear_probe_dataset_objs(
    dataset_name,
    model_name,
    label,
    func,
    ...
    df_train, df_val, df_test, case_name, _ = get_data_info(
        dataset_name=dataset_name, prefix=prefix, usecolumns=[label]
    )

    if string_convert:
        dict_train = convert_keys_to_strings(
            joblib.load(
                f"{prefix}../data/{dataset_name}/features/{model_name}/dict_train{content}.p"
            )
        )
        dict_val = convert_keys_to_strings(
            joblib.load(
                f"{prefix}../data/{dataset_name}/features/{model_name}/dict_val{content}.p"
            )
        )
        dict_test = convert_keys_to_strings(
            joblib.load(
                f"{prefix}../data/{dataset_name}/features/{model_name}/dict_test{content}.p"
            )
        )
    else:
        dict_train = joblib.load(
            f"{prefix}../data/{dataset_name}/features/{model_name}/dict_train{content}.p"
        )
        dict_val = joblib.load(
            f"{prefix}../data/{dataset_name}/features/{model_name}/dict_val{content}.p"
        )
        dict_test = joblib.load(
            f"{prefix}../data/{dataset_name}/features/{model_name}/dict_test{content}.p"
        )
    ...
    X_train, y_train, train_keys = func(
        df=df_train,
...
def load_linear_probe_dataset_combined(
    model_name,
    label,
    func,
    content,
    ...
    if concat:
        X_train_vital, y_train_vital, X_test_vital, y_test_vital, _, _, _ = (
        ...
        X_train_mesa, y_train_mesa, X_test_mesa, y_test_mesa, _, _, _ = (
            load_linear_probe_dataset_objs(
                dataset_name="mesa",
                model_name=model_name,
                label=label,
        ...
        X_train_mimic, y_train_mimic, X_test_mimic, y_test_mimic, _, _, _ = (
            load_linear_probe_dataset_objs(
                dataset_name="mimic",
                model_name=model_name,
                label=label,
