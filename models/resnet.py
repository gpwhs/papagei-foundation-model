...
class ResNet1DMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # (batch_size, n_experts, 1)
        gate_weights_1 = self.gating_network_1(out)  # (batch_size, n_experts)
        out_moe1 = torch.sum(
            gate_weights_1.unsqueeze(2) * expert_outputs_1, dim=1
        ...
        out_moe2 = torch.sum(
            gate_weights_2.unsqueeze(2) * expert_outputs_2, dim=1
        )  # Weighted sum of experts
...
class TFCResNet(nn.Module):
    def __init__(self, model_config):
        ...
        self.resnet_encoder_t = ResNet1DBackBone(
            in_channels=1,
        ...
        self.resnet_encoder_f = ResNet1DBackBone(
            in_channels=1,
            base_filters=model_config["base_filters"],
            kernel_size=model_config["kernel_size"],
            stride=model_config["stride"],
